...
// necessary to enable inheritance
...
// Cities
var city_locations = [{x:47.5, y:36, name:"France"},	//0
					  {x:46.5, y:31.5, name:"United Kingdom"},
					  {x:85.5, y:43, name:"Japan"},
					  {x:25, y:40, name:"United States (East Coast)"},
					  {x:49.2, y:32.5, name:"Germany"},
					  {x:85, y:78, name:"Australia"},			
					  {x:18, y:30, name:"Canada"},
					  {x:40, y:18.5, name:"Greenland"},
					  {x:13, y:40, name:"United States (West Coast)"},
					  {x:78, y:43, name:"China"},					  				
					  {x:43, y:75, name:"none"}];	
...
//draws circles on the continents 
var CircleAnim = inherit(function(regionType, x, y) {
	...
	drawSpeechBubble: function(){	
		...
		for(k = 0; k < city_locations.length; k++)
		{			
			xcoord = city_locations[k]["x"]*width/100;
			ycoord = city_locations[k]["y"]*height/100;
			if(mouseX <= xcoord+dist && mouseX >= xcoord-dist && mouseY <= ycoord+dist && mouseY >= ycoord-dist)
			{
				noStroke();
				fill(#FFFFFF, 100);			
				triangle(xcoord,ycoord,xcoord+(50*i),ycoord+(0*i),xcoord+(50*i),ycoord+(10*i));					
				if(i < 1)
				{
					i += 2*rate;
				}
				...
				{
					rectMode(CORNER);
					rect(xcoord+50, ycoord+0, 120*j, 80*j);
					place = "Information about \n" + city_locations[k]["name"];					
					if(j < 1)
					{
						j += rate;
					}
					else
					{
						fill(#555555, 200)
						textFont("Oswald", 10);
						text(place, xcoord+115, ycoord+20);
					}
...
var CurveAnim = inherit(function(curve, curveSet) {
	...
	drawSmiley: function() {
        ...
        if (0 === curve.points.length) {
            alpha = alpha * (1 - 1.5*(this.time - DEFAULT_ANIMATION_LENGTH) / DEFAULT_ANIMATION_LENGTH);
            this.time += 1;
			i += (this.time - DEFAULT_ANIMATION_LENGTH) / DEFAULT_ANIMATION_LENGTH;
        }        
		
		//draw smiley		
				translate(curve.supplierX, curve.supplierY);	
				stroke(0, 0, 0, .5*alpha);				
				strokeWeight(.1);
				fill(233, 224 , 71, .5*alpha);
				ellipse(0, 0, 15*i, 15*i);
				strokeWeight(1);
				fill(0, .5*alpha);
				ellipse(-2.5*i, -2*i, 1.5*i, 3.5*i);
				fill(0, .5*alpha);
				ellipse(2.5*i, -2*i , 1.5*i, 3.5*i);
				fill (233, 224 , 71, .5*alpha);
				bezier(-5*i, 1.5*i, -2.5*i, 6*i, 2.5*i, 6*i, 5*i, 1.5*i);			   	
			resetMatrix(); 
    },
...
var SloganAnim = inherit(function() {}, AnimationClass, {					//Do something fun with this later
    data: {
        text: ['WELCOME TO THE NETWORKED ECONOMY', '', '',
